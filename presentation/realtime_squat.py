"""
Gercek zamanli squat form analiz ornegi.
Bu moduldaki akim/arayuz, katmanli yapida presentation katmanina tasindi.
"""
...
if not os.path.exists(MODEL_FILE):
    print(f"[INDIRILIYOR] {MODEL_FILE} — lutfen bekleyin...")
    try:
...
def draw_panel(frame, metrics):
    for i, m in enumerate(metrics):
        ...
        cv2.putText(frame, m["label"], (x1 + 12, y1 + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.46, C_TEXT, 1, cv2.LINE_AA)
        val = f"{m['v']:.1f}°" if isinstance(m["v"], float) else str(m["v"])
        cv2.putText(frame, val, (x1 + 12, y1 + 54), cv2.FONT_HERSHEY_DUPLEX, 0.9, m["c"], 2, cv2.LINE_AA)
...
def draw_header(frame, reps, status, color):
    ...
    cv2.putText(frame, "BIYOMEKANIK FORM ANALIZORU", (12, 28), cv2.FONT_HERSHEY_DUPLEX, 0.65, C_GOLD, 1, cv2.LINE_AA)
    cv2.putText(frame, f"TEKRAR: {reps}", (12, 58), cv2.FONT_HERSHEY_SIMPLEX, 0.52, C_BLUE, 1, cv2.LINE_AA)
    ts = cv2.getTextSize(status, cv2.FONT_HERSHEY_DUPLEX, 0.7, 2)[0]
    cv2.putText(frame, status, (w - ts[0] - 15, 42), cv2.FONT_HERSHEY_DUPLEX, 0.7, color, 2, cv2.LINE_AA)
...
def draw_hints(frame):
    ...
    for i, t in enumerate(["Q: Cikis", "R: Sifirla", "S: Goruntu"]):
        cv2.putText(frame, t, (w - 160, h - 70 + i * 22), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (110, 110, 130), 1, cv2.LINE_AA)
...
def main():
    ...
    print("=" * 58)
    print("  Biyomekanik Form Analizoru BASLADI")
    print("  Tam vucudunuz kameraya gorundugunden emin olun.")
    print("  Q: Cikis  |  R: Tekrar sifirla  |  S: Ekran goruntusu")
    print("=" * 58)
    ...
    with PoseLandmarker.create_from_options(opts) as detector:
        while True:
            ...
            if result.pose_landmarks:
                ...
                cv2.putText(
                    frame,
                    "Poz algilanamadi – tam vucudunuz gorunsun!",
                    (60, h // 2),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.7,
                    C_WARN,
